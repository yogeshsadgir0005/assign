"use client";

import { useCallback, useEffect, useState } from "react";
import { fetchProduct } from "@/api/products";
import type { Product } from "@/types/product";
import { ApiError, isCanceled } from "@/lib/api-error";
import { findLocalProduct, isLocalId, useOverrides } from "@/lib/overrides";

type Result = { key: string; product: Product | null; error: ApiError | null };

const NOTHING_YET: Result = { key: "", product: null, error: null };
const NOT_A_PRODUCT = new ApiError("not-found", "That product id isn't a number.", 404);
const LOCAL_GONE = new ApiError("not-found", "This product was only ever local, and it's gone.", 404);

export function useProduct(id: number) {
  const overrides = useOverrides();
  const [result, setResult] = useState<Result>(NOTHING_YET);
  const [attempt, setAttempt] = useState(0);

  const valid = Number.isInteger(id) && id >= 1;
  // Products added in this session exist only locally — the API would 404.
  const local = valid && isLocalId(id);
  const key = valid && !local ? `${id}#${attempt}` : "";

  const retry = useCallback(() => setAttempt((value) => value + 1), []);

  useEffect(() => {
    if (!key) return;
    const controller = new AbortController();
    let current = true;

    fetchProduct(id, { signal: controller.signal })
      .then((product) => {
        if (current) setResult({ key, product, error: null });
      })
      .catch((error: ApiError) => {
        if (!current || isCanceled(error)) return;
        setResult({ key, product: null, error });
      });

    return () => {
      current = false;
      controller.abort();
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [key]);

  if (!valid) {
    return {
      status: "error" as const,
      product: null,
      error: NOT_A_PRODUCT,
      deleted: false,
      edited: false,
      retry,
    };
  }

  if (local) {
    const found = findLocalProduct(overrides, id);
    return {
      status: found ? ("ready" as const) : ("error" as const),
      product: found,
      error: found ? null : LOCAL_GONE,
      deleted: !found,
      edited: true,
      retry,
    };
  }

  const settled = result.key === key;
  const edited = overrides.updated[id];

  return {
    status: (settled ? (result.error ? "error" : "ready") : "loading") as
      | "loading"
      | "ready"
      | "error",
    product: settled && result.product && edited ? { ...result.product, ...edited } : settled ? result.product : null,
    error: settled ? result.error : null,
    deleted: overrides.deleted.includes(id),
    edited: Boolean(edited),
    retry,
  };
}
